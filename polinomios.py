...
def main():
  ...
  while True:
    ...
    if select == 1:
      ...
      for k, v in p.constantes.iteritems():
        print("Constante del termino x^" + str(k) + " : "  + str(v))
    elif select == 3:
      x = ''
      while True:
        x = raw_input('Que Valor de x? (s para terminar)')
        if x == 's':
          break
        p.evaluar(x)
main()